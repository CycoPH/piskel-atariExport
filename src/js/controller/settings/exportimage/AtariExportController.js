(function () {
  ...
  ns.AtariExportController.prototype.fromPaletteExport_ = function () {
    ...
    // If the palette contains just one color then that is taken to be the reference color that will
    // be used for creating the bit plane.
    // if the palette contains 3 (or more) colors then colors 0,1,2 are taken to be the bit
    // values (0 = plane 1, 1 = plane 2, 2 = both planes)
  ...
  ns.AtariExportController.prototype.fromLayersExport_ = function () {
    ...
    for (var layerNr = 0; this.piskelController.hasLayerAt(layerNr); layerNr++) {
      if (layerNr >= 2) {
        break;            // Too many layers! We only look at the first 2
      }
